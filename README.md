...
- [CamoDB : v1.2](#camodb--v12)
  ...
  - [Support](#support)
## What is it?:
CamoDB is a super simple database backend solution that uses `PHP` and `JSON` files to efficiently store your project's data. Being built with simple projects in mind, it's a quick and easy way to store data for low traffic websites and hobby projects.
...
## Getting started
#### How do I install it?
The installation process is very simple! Just create a new, empty, folder inside your project files and dump all the files in this repository into it. Everything else can be done through the admin panel and a text editor of your choice.
...
To configure it, click "Manage" on the database you want to configure. You'll be greeted with a page that has these options:
...
- `Require Auth?` determines whether or not the database requires authentication to access. If you set this to true, only accounts that have been created in the users list can access the database. If you set this to false, anyone can access the database regardless of whether or not they have an account.
...
---
## Access Levels
Access levels are a way of controlling who can access the database. There are 4 levels of access to choose from. While databases are recommended to have the following access levels, you can configure them however you want.
...
- `0` - Minimum access level. (Recommended for read only access)
- `1` - Medium access level. (Recommended for read and write access)
- `2` - Maximum access level. (Recommended for read, write and delete access)
...
###### CALLS Path
The `path` field is used to specify the path to the data to read, write or delete. It is a string that is formatted as a directory path. Each part of the path should be separated by a backslash `/` and should not start or end with a backslash. The path is relative to the `db` folder and each part of it is case sensitive.
...
- `set` - Sets the data at the specified path.
  ...
  - If the entry of collection does not exist, it will be created. Databases cannot be created with this action.
...
```json
{
    ...
    "CALLS":[
        {
            "path": "myDatabase/collection1/myEntry1",
            "action": "set",
            "data": {
        ...
        {
            "path": "myDatabase/collection1/myEntry2",
            "action": "set",
            "data": {
                "anotherField": "Hi, I’m Camo",
